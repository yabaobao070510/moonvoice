...
**把录音切成"可识别"的语音段，并给出格式 / 电平 / 特征——纯 MoonBit，零依赖，可沙箱运行。不做语音增强。**
...
打开后可用预置的**办公室**场景（AMI 真实会议录音），或选一段你自己的录音（**本地处理、
不上传**）。页面把音频喂进浏览器内的 wasm，依次执行 VAD 分段、重采样、响度测量和组合链，
给出成品音频与每个技能的实测数据：

| 技能 | 页面里展示的结果 |
|---|---|
| `cmd/vad` | 检出几段语音、语音占比，段边界标在波形顶部标尺上，附完整段表 |
| `cmd/resample` | 采样率变化与时长保持 |
| `cmd/loudness` | 积分响度（LUFS）、峰值、到目标响度所需增益 |
| 组合链 | 分段 + 16 kHz + 单声道 + 归一 -24 LKFS 的成品音频；**对成品重新测响度并统计削波样本**（闭环验证） |

两个波形使用**同一幅度刻度**：原录音电平过轻时波形明显更薄——这就是要解决的问题，
归一化后的成品可以直接听出差别。
...
实测（无头 Chrome，办公室 60 s 素材；模块在页面加载后已预热）：

| 任务 | 耗时 |
|---|---|
| `cmd/vad`（分段） | 448 ms |
| `cmd/resample`（48k→16k） | 405 ms |
| `cmd/loudness`（EBU R128 测量） | 141 ms |
| 组合链（分段 + 单声道 + 归一 -24）＋成品复测 | 506 ms |
| **合计** | **1508 ms（40× 实时）** |

> 预热是必要的：浏览器 wasm 引擎分层编译，模块**首次执行**慢约 8 倍
> （实测首次 1.7 s、热后 0.2 s）。不预热的话，报出的是启动开销而不是处理开销。

素材（真实录音，已重采样为 48 kHz 立体声演示格式）：AMI Meeting Corpus `ES2002a.Mix-Headset`
（**CC BY 4.0**）——真实会议室多人对话，取 501.7 s 起的 60 s 窗口（全场实测 -40.7 LUFS，
典型"电平过轻"录音）。

> 范围说明：本版不做语音增强（不改变信号）——页面展示的是**分段 / 格式 / 电平**；
> 复杂噪声场景的边界见「已知边界」。

重新生成：
```bash
python demo/build_materials.py                              # 原始录音 → 切片/转码出嵌入用 MP3
...
- **不做语音增强 / 人声析出**：VAD 只回答"哪里是语音"，不改变信号——噪声只会随重采样和响度
  归一一起被处理，不会"削弱噪声、提高人声"。而且当噪声大到盖过人声、**没有有效的"噪声底"
  可跟踪**时，该判定本身也会退化（实测：公交 / 街道等持续人声实录音上检出占比 99–100%）。
  强噪声场景的语音增强在 v0.2 路线图。
