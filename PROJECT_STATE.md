...
- **阶段**：D1 完成（audio 包 + 金标对拍全绿）；下一步 D2 重采样
- **赛期**：2026 MoonBit 黑客松 · 九月赛，**2026-09-30 24:00 截止报名与验收**（剩 9 天）
- **进度**：23/23 测试三后端（wasm/js/native）全绿；`moon check` 0 警告
- **阻塞项**（只剩账号，需用户本人）：
  1. `moon register` / `moon login`（交互式）—— 发布技能前必须
...
| 日 | 内容 | 证据 |
|---|---|---|
| D0 9/21 | 工具链安装与验证、仓库骨架、R1/R2 风险打掉 | `moon test` 三后端可跑 |
| D1 9/21 | `audio` 包：WAV/RIFF 编解码 + 样本表示 | 14 项单测 + 8 项金标，三后端全绿 |

**金标对拍已建立**（这是评委关心的"测试质量"的抓手）：
- fixture：ffmpeg 生成 7 个格式（u8/s16/s24/s32/f32/f64/立体声）
- 期望值：libsndfile（第三方独立解码器）—— 不与被测实现共享任何代码
- 容差有理论依据：u8/s16/s24/f32 的样本值为 k/2^n（n≤24）在 f32 中精确 → 容差 0；s32/f64 理论相对误差 ≤ 2^-24 → 取 1e-7
- 复现：`python tools/golden/gen_audio_golden.py`
